# -*- coding: utf-8 -*-
"""
Created on Sat Mar  2 18:49:48 2024
...
# im_cam = Image.open("/Users/patricia.roemer/Desktop/PhD/Python/Background/cameraman.tif")
...
im = zoom(im_cam[:,:],factor)

# obj = image_to_object(im,lambda x,v: x)

obj = im * np.exp(2.0j*np.pi * im / 256.0)
...
noise *= 0.01*np.sqrt(np.sum(b**2)) / np.sqrt(np.sum(np.abs(noise)**2)) 
...
locations_old = par.locations + 0
factor = par.locations // shift
par.locations += factor *0.25
...
af_alg = af.amplitude_flow(
    ...
    AG_params = AG_par,
    number_of_iterations = 300, 
    grad_threshold = 10**-10,
    # learning_rate_type = 'custom',
    # learning_rate = 0.01,
    # epsilon = 10**-8,
    # alpha_T = 10**0.5,
    # alpha_R = 0,
...
locations_shifted = par.locations + 0
new_locations = af_alg.run_shifts(obj)
