# -*- coding: utf-8 -*-
"""
Created on Wed Dec  2 17:52:31 2020
...
# from find_largest_divisor_of_n_less_eq_than_r import find_largest_divisor_of_n_less_eq_than_r
...
class wdd:
    def __init__(self, 
        ...
        # self.nongrid_shifts = False
        # if not hasattr(self.par, 'shift') :            
        #     self.nongrid_shifts = True
        #     R = self.par.locations.shape[0]
        #     ds0 = find_largest_divisor_of_n_less_eq_than_r(self.par.object_shape[0],np.sqrt(R))
        #     s_0 = self.par.object_shape[0] // ds0
            
        #     ds1 = find_largest_divisor_of_n_less_eq_than_r(self.par.object_shape[1],np.sqrt(R))
        #     s_1 = self.par.object_shape[1] // ds1
            
        #     self.shift = (s_0.astype(int), s_1.astype(int))
            
        #     print('Found non-grid shifts. Using ', self.shift, 'downsampling. Requires pynfft library.')
        #     import pynfft
        # else:
        
        if hasattr(self.par, 'shift'):
            self.shift = self.par.shift 
        else:
            self.shift = (1,1)
            print('WARNING: ptycho has no atribute shift. Use NFFT version instead.')
        ...
        # if 'nfft_diags' in kwargs.keys():
        #     if isinstance(kwargs['nfft_diags'], bool):
        #         self.nfft_diags = kwargs['nfft_diags']
        #     else:
        #         self.nfft_diags = False
        # else:    
        #     self.nfft_diags = False
        ...
        self.dim_c = (self.par.object_shape[0]//self.shift[0],self.par.object_shape[1]//self.shift[1])
        self.dim_c_ext = (self.object_shape_ext[0]//self.shift[0],self.object_shape_ext[1]//self.shift[1])
        self.wnd_size_c = (self.par.window_shape[0]//self.shift[0],self.par.window_shape[1]//self.shift[1])
    ...
    def compute_singular_values(self):
        ...
        for k0 in range(self.wnd_size_c[0]):
            ...
            for k1 in range(self.wnd_size_c[1]):
                ...
                w1 = self.par.window[k0*self.shift[0]:,k1*self.shift[1]:]
                ...
                for n0 in range(self.wnd_size_c[0]):
                    for n1 in range(self.wnd_size_c[1]):
                        w[n0,n1] = np.sum(w2[n0*self.shift[0]:(n0+1)*self.shift[0],n1*self.shift[1]:(n1+1)*self.shift[1]],axis =None)
            ...
            for k1 in range(self.wnd_size_c[1] - 1):
                ...
                w1 = self.par.window[k0*self.shift[0]:,:(self.par.window_shape[1]-(k1+1)*self.shift[1])]
                w2 = self.par.window[:w1.shape[0],:].conj()
                w2[:,:(self.par.window_shape[1]-w1.shape[1])] = 0
                w2[:,(self.par.window_shape[1]-w1.shape[1]):] *= w1
                
                # w4 = np.zeros( (self.dim_c_ext[0],self.dim_c_ext[1]), dtype = complex)
                # w4[:self.wnd_size_c[0],:self.wnd_size_c[1]] = self.par.window
                # w4 = np.roll(np.roll(w4, k0*self.par.shift[0], axis = 0), (k1+1)*self.par.shift[1], axis = 1)
                # w4[:self.wnd_size_c[0],:self.wnd_size_c[1]] *= self.par.window.conj() 
                ...
                for n0 in range(self.wnd_size_c[0]):
                    for n1 in range(self.wnd_size_c[1]):
                        w[n0,n1] = np.sum(w3[n0*self.shift[0]:(n0+1)*self.shift[0],n1*self.shift[1]:(n1+1)*self.shift[1]],axis =None)
                w_fft = np.fft.fft2(w.conj())
                w_fft = w_fft.conj()    
    ...
    def construct_diagonals_fft(self):
        ...
        if (self.par.circular == False):
            nshifts = ( 1 + (self.par.object_shape[0]-self.par.window_shape[0])//self.shift[0],1+(self.par.object_shape[1]-self.par.window_shape[1])//self.shift[1])
        else:
            nshifts = ( (self.par.object_shape[0])//self.shift[0],(self.par.object_shape[1])//self.shift[1])
        ...
        for k0 in range(self.wnd_size_c[0]):
            ...
            for k1 in range(self.wnd_size_c[1]):
                ...
                b_cur = b_iift[k0 * self.shift[0],k1*self.shift[1],:]
                b_2d = diff_pat_ifft_dummy[k0 * self.shift[0],k1*self.shift[1]]*np.ones(self.dim_c_ext,dtype= complex)
                b_2d[:nshifts[0],:nshifts[1]] = np.reshape(b_cur,nshifts)
                b_2d_fft = np.fft.fft2(b_2d)
                        
                # above_threshold = np.abs(self.singular_values[:,:,k0,k1]) > self.reg_param
                above_threshold = self.above_threshold[:,:,k0,k1]
                # thresholds[:,:,k0,k1] = np.abs(self.singular_values[:,:,k0,k1])
            ...
            for k1 in range(self.wnd_size_c[1]-1):
                ...
                # Apply location-wise Fourier transform
                b_cur = b_iift[k0 * self.shift[0],self.par.fourier_dimension[1]- (k1+1)*self.shift[1],:]
                b_2d = diff_pat_ifft_dummy[k0 * self.shift[0],self.par.fourier_dimension[1]- (k1+1)*self.shift[1]]*np.ones(self.dim_c_ext,dtype= complex)
                b_2d[:nshifts[0],:nshifts[1]] = np.reshape(b_cur,nshifts)
                b_2d_fft = np.fft.fft2(b_2d)
                ...
                # tr_diag *= self.xt
                # tr_diag_fft = np.fft.fft2(tr_diag)
    ...
    def construct_diags(self):
        ...
        # if self.nfft_diags:
        #     diags = np.zeros_like(self.diags_fft)
            
        #     for k1 in range(self.wnd_size_c[0]):
        #         for k2 in range(self.wnd_size_c[1]):
        #             idx = np.where(self.above_threshold[:,:,k1,k2])
                    
        #             if len(idx[0]) == 0:
        #                 continue
                    
        #             x = np.zeros((len(idx[0]),2))
        #             x[:,0] = 1.0*idx[0] / self.dim_c_ext[0]
        #             x[:,1] = 1.0*idx[1] / self.dim_c_ext[1]
        #             x[x > 0.5] -= 1  
                    
        #             plan = pynfft.nfft.NFFT([self.dim_c_ext[0], self.dim_c_ext[1]], len(idx[0]))
        #             plan.x = x
        #             plan.precompute()
        #             plan.f = self.diags_fft[self.above_threshold[:,:,k1,k2],k1,k2] / np.prod(self.dim_c_ext)
        #             diags[:,:,k1,k2] = np.fft.fftshift(plan.adjoint())
        ...
        # t1 = self.diags[:,:,12,13]
        # t2 = self.diags[:,:,12,11]
        # np.sum(np.abs(np.roll(t1,1,axis = 1) - t2.conj()))
    ...
    def reconstruct_magnitudes_from_lifted_matrix(self):
        ...
        if self.mg_type == 'diag':
            ...
            if self.mg_diagonals_type == 'value':
                gamma0 = np.maximum(self.mg_diagonals_param[0] // self.shift[0],1)
                gamma1 = np.maximum(self.mg_diagonals_param[1] // self.shift[1],1)
                # gamma = (gamma0,gamma1)
                used_diags[gamma0:,:] = 0
                used_diags[:,gamma1:] = 0
    ...
    def construct_object(self):
        ...
        x = np.repeat(x_compressed_cutted, self.shift[0], axis = 0)
        x = np.repeat(x, self.shift[1], axis = 1)
