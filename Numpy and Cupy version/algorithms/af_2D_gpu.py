...
#         self.track_it = np.sort(track_it)        
    ...
    def learn_rate_2D_pty_optimal(self):
        ...
        learn_rate = 2*(1-self.AG_params.control)*learn_rate**-1
    ...
    def objective_L2_2D(self,z):
        ...
        sqbx = cp.sqrt( cp.abs(fz)**2 + self.epsilon) 
        objective = cp.sum(cp.abs(sqbx - self.sqb)**2, axis = None)
