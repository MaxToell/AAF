# -*- coding: utf-8 -*-
"""
Created on Wed Sep 30 12:32:39 2020
...
class amplitude_flow:
    def __init__(self,
        ...
        if 'measurements' in kwargs.keys():
            self.sqb = np.sqrt(np.maximum(kwargs['measurements'],0) + self.epsilon)    
        ...
        if self.track:
            ...
            if 'track_it' in kwargs.keys():
                self.track_it = np.sort(kwargs['track_it'])
            else:
                self.track_it = []
        else:
            self.track_it = []
...
#         self.track_it = np.sort(track_it)        
    ...
    def fast_grad_L2_2D_pty(self,forw, sqrt_meas):
        ...
        for r in range(self.R):
            ...
            grad_r = np.zeros(self.par.object_shape,dtype = complex)
            grad_r[:self.delta1,:self.delta2] = self.fast_grad_AF_2D_os(forw[:,:,r],sqrt_meas[:,:,r],self.sqb[:,:,r])
            # grad_r = np.roll(grad_r, loc[0], axis =0)
            # grad_r = np.roll(grad_r, loc[1], axis =1)
            grad_r = self.par.shift_vec(grad_r,loc)

            grad += grad_r
                
        return grad 

    def fast_grad_L2_win_2D_pty(self,forw,sqrt_meas):
        grad = np.zeros((self.delta1,self.delta2),dtype = complex)
            
        z = self.par.obj.copy()
        for r in range(self.R):
            loc = self.par.locations[r,:]
            z_r = z.copy()
            # z_r = np.roll(z_r, -loc[0], axis =0)
            # z_r = np.roll(z_r, -loc[1], axis =1)
            z_r = self.par.shift_vec(z_r,-loc)
    ...
    def grad_smoothness_Tikhonov(self,z):
        if self.par.circular:
                gr = (4*z - np.roll(z,1,axis =0) - np.roll(z,-1,axis =0) - np.roll(z,1,axis =1) - np.roll(z,-1,axis =0))
        else:
            gr = np.zeros_like(z)
    ...
    def grad_TV_smoothed(self,z):
        if self.par.circular:
            df  = z - np.roll(z,1,axis = 0)
            gr = df / np.sqrt(df**2 + self.TV_param)
            df  = z - np.roll(z,-1,axis = 0)
            gr += df / np.sqrt(df**2 + self.TV_param)
            df  = z - np.roll(z,1,axis = 1)
            gr += df / np.sqrt(df**2 + self.TV_param)
            df  = z - np.roll(z,-1,axis = 1)
            gr += df / np.sqrt(df**2 + self.TV_param)
        else:
            ...
            gr[:z.shape[0]-1,:] += df / np.sqrt(df**2 + self.TV_param)
            df = z[1:,:] - z[:z.shape[0]-1,:]
            gr[1:,:] += df / np.sqrt(df**2 + self.TV_param)
            df = z[:,:z.shape[1]-1] - z[:,1:]
            gr[:,:z.shape[1]-1] += df / np.sqrt(df**2 + self.TV_param)
    ...
    def grad_AF_2D_pen(self,z):
        ...
        if self.alpha_R != 0:
            grad += self.alpha_R* 1.0j* np.imag(z)
    ...
    def fast_grad_AF_2D_pty_win(self,w,forw,sqrt_meas):
    ...
    def learn_rate_penalties(self):
        ...
        if self.par.circular:    
            ...
            learn_rate += self.alpha_ST * (8 + 2* np.sum(np.sqrt(self.par.object_shape)))
        
        # smoothed TV penalty
        if self.alpha_TV != 0 and not self.proximal_TV:
            if self.par.circular:    
                learn_rate += self.alpha_TV * 0.5*8 / np.sqrt(self.TV_param)
            else:
                learn_rate += self.alpha_TV * 0.5*(8 + 2* np.sum(np.sqrt(self.par.object_shape))) / np.sqrt(self.TV_param) 
    ...
    def learn_rate_penalties_win(self):
        ...
        if self.par.circular:    
            ...
            learn_rate += self.alpha_ST * (8 + 2*np.sqrt(self.delta1) + 2* np.sqrt(self.delta2))
        ...
        if self.alpha_TV != 0 and not self.proximal_TV:
            if self.par.circular:    
                ...
                learn_rate += self.alpha_TV * 0.5*(8 + 2* np.sqrt(self.delta1) + 2* np.sqrt(self.delta2)) / np.sqrt(self.TV_param) 
                
        return learn_rate

    def learn_rate_2D_pty_optimal(self):
        abs_win = np.abs(self.par.window)**2
        temp = np.zeros(self.par.object_shape)
        for r in range(self.R):
            loc = self.par.locations[r,:]
            # temp[loc[0]:(loc[0] + delta1),loc[1]:(loc[1] + delta2)] += abs_win
            win_t = np.zeros(self.par.object_shape)
            win_t[:self.delta1,:self.delta2] = abs_win
            # win_t = np.roll(win_t, loc[0], axis=0)
            # win_t = np.roll(win_t, loc[1], axis=1)
            win_t = np.abs(self.par.shift_vec(win_t,loc))
            
            temp += win_t
        
        max_win = np.max(temp)
        
        learn_rate = max_win*np.prod(self.par.fourier_dimension)
    ...
    def learn_rate_2D_pty_sub_optimal(self):
        ...
        max_win = self.R*np.max(np.abs(self.par.window)**2)
    ...
    def learn_rate_2D_pty_win_optimal(self):
        ...
        temp = np.zeros(self.par.window_shape)
        for r in range(self.R):
            loc = self.par.locations[r,:]
            z_r = z.copy()
            # z_r = np.roll(z_r, -loc[0], axis=0)
            # z_r = np.roll(z_r, -loc[1], axis=1)
            z_r = self.par.shift_vec(z_r,-loc)
            temp += np.abs(z_r[:self.delta1,:self.delta2])**2     
    ...
    def learn_rate_2D_pty_win_sub_optimal(self):
        z = self.par.obj   
        max_win = self.R * np.max(np.abs(z))
    ...
    def objective_L2_2D(self,z):
        ...
        sqbx = np.sqrt( np.abs(fz)**2 + self.epsilon) 
        objective = np.sum(np.abs(sqbx - self.sqb)**2, axis = None)
        return objective, fz, sqbx
    
    # || (I - S) z||_2^2
    def objective_smoothness_Tikhonov(self,z):
        if self.par.circular:
            obj = np.linalg.norm(z - np.roll(z,1,axis =0),'fro')**2
            obj += np.linalg.norm(z - np.roll(z,-1,axis =0),'fro')**2
            obj += np.linalg.norm(z - np.roll(z,1,axis =1),'fro')**2
            obj += np.linalg.norm(z - np.roll(z,-1,axis =1),'fro')**2
        else:
            obj = np.linalg.norm(z[:z.shape[0]-1,:] - z[1:,:],'fro')**2
            obj += np.linalg.norm(z[1:,:] - z[:z.shape[0]-1,:],'fro')**2
            obj += np.linalg.norm(z[:,:z.shape[1]-1] - z[:,1:],'fro')**2
            obj += np.linalg.norm(z[:,1:] - z[:,:z.shape[1]-1],'fro')**2
            
        return obj
    
    def objective_TV_smoothed(self,z):
        if self.par.circular:
            obj = np.sum(np.sqrt(np.abs(z - np.roll(z,1,axis =0))**2 + self.TV_param))
            obj += np.sum( np.sqrt(np.abs(z - np.roll(z,-1,axis =0))**2 + self.TV_param))
            obj += np.sum( np.sqrt(np.abs(z - np.roll(z,1,axis =1))**2 + self.TV_param))
            obj += np.sum( np.sqrt(np.abs(z - np.roll(z,-1,axis =1))**2 + self.TV_param))
        else:
            obj = np.sum(np.sqrt(np.abs(z[:z.shape[0]-1,:] - z[1:,:])**2 + self.TV_param))
            obj += np.sum(np.sqrt(np.abs(z[1:,:] - z[:z.shape[0]-1,:])**2 + self.TV_param))
            obj += np.sum(np.sqrt(np.abs(z[:,:z.shape[1]-1] - z[:,1:])**2 + self.TV_param))
            obj += np.sum(np.sqrt(np.abs(z[:,1:] - z[:,:z.shape[1]-1])**2 + self.TV_param))
    ...
    def objective_pen_2D(self,z):
        ...
        if self.alpha_T != 0:
            objective += self.alpha_T*np.sum(np.abs(z)**2, axis = None)
        
        if self.alpha_R != 0:
            objective += self.alpha_R*np.sum(np.imag(z)**2, axis = None)
    ...
    def run_win(self,w_0, obj_shift=0.0):
        grad_f = lambda w, forw,sqrt_meas : self.fast_grad_AF_2D_pty_win(w,forw,sqrt_meas)
    ...
    def obj_shift(self,loc,z,sqb_loc):
        z_r = self.par.shift_vec(z, -loc)
        z_r = z_r[:self.delta1,:self.delta2]
        fz = self.par.forward_2D_os(z_r)
        sqbx = np.sqrt( np.abs(fz)**2 + self.epsilon) 
        obj = np.sum(np.abs(sqbx - sqb_loc)**2, axis = None)
        return obj, obj, fz, sqbx
    
    def grad_shift(self,loc,forw,sqrt_meas,sqb_loc,z,fft_z_c):
        grad = np.zeros_like(z)
        grad[:self.delta1,:self.delta2] = self.fast_grad_AF_2D_os(forw, sqrt_meas, sqb_loc)
        grad = np.fft.fft2(grad) / np.prod(z.shape)
        grad = grad * fft_z_c
        t = np.ones_like(z,dtype = complex)
        t = np.swapaxes(np.swapaxes(t, 0, 1) *np.exp( 2j*np.pi*-loc[0]*np.arange(z.shape[0]) / z.shape[0]), 0,1) 
        t = t * np.exp( 2j*np.pi*-loc[1]*np.arange(z.shape[1]) / z.shape[1])
        gr_1 = np.swapaxes(np.swapaxes(grad, 0, 1) * -2j*np.pi*np.arange(z.shape[0]), 0,1) / z.shape[0]
        gr_2 = grad * -2j*np.pi*np.arange(z.shape[1]) / z.shape[1]
        gr_loc1 = np.real(np.sum(t* gr_1,axis = None))
        gr_loc2 = np.real(np.sum(t* gr_2,axis = None))
        gr = np.array([gr_loc1,gr_loc2])
        return(gr)
    ...
    #     q = np.ones_like(z,dtype = complex)
    #     q = np.swapaxes(np.swapaxes(q, 0, 1) *np.exp( 2j*np.pi*loc[0]*np.arange(z.shape[0]) / z.shape[0]), 0,1) 
    #     q = q * np.exp( 2j*np.pi*loc[1]*np.arange(z.shape[1]) / z.shape[1])
        
    #     D1 = lambda x: np.swapaxes(np.swapaxes(x, 0, 1) * 2j*np.pi*np.arange(z.shape[0]), 0,1) / z.shape[0]
    #     D2 = lambda x: x * 2j*np.pi*np.arange(z.shape[1]) / z.shape[1]
        
    #     D1q  = fft_z_c.conj() * D1(q)
    #     D1q  = np.fft.ifft2(D1q)
    #     AD1q  = np.zeros_like(z)
    #     AD1q[:self.delta1,:self.delta2] = self.par.window * D1q[:self.delta1,:self.delta2]
    #     AD1q  = np.fft.fft2(AD1q)
        
    #     D2q  = fft_z_c.conj() * D2(q)
    #     D2q  = np.fft.ifft2(D2q)
    #     AD2q  = np.zeros_like(z)
    #     AD2q[:self.delta1,:self.delta2] = self.par.window * D2q[:self.delta1,:self.delta2]
    #     AD2q  = np.fft.fft2(AD2q)
        
    #     diag_1 = 1 - sqb_loc / sqrt_meas + 0.5*sqb_loc * np.abs(forw)**2 / sqrt_meas**1.5  
    #     diag_2 = 0.5*sqb_loc * forw**2 / sqrt_meas**1.5
        
    #     H = np.zeros( (2,2))
        
    #     H[0,0] =  np.real(np.sum(D1(D1(q)).conj() * grad,axis = None))
    #     H[0,0] += 2 * np.real( np.sum( AD1q.conj() * diag_1 * AD1q))
    #     H[0,0] += 2 * np.real( np.sum( AD1q.conj() * diag_2 * AD1q.conj()))
        
    #     H[0,1] =  np.real(np.sum(D2(D1(q)).conj() * grad,axis = None))
    #     H[0,1] += 2 * np.real( np.sum( AD1q.conj() * diag_1 * AD2q))
    #     H[0,1] += 2 * np.real( np.sum( AD1q.conj() * diag_2 * AD2q.conj()))
        
    #     H[1,0] =  np.real(np.sum(D2(D1(q)).conj() * grad,axis = None))
    #     H[1,0] += 2 * np.real( np.sum( AD2q.conj() * diag_1 * AD1q))
    #     H[1,0] += 2 * np.real( np.sum( AD2q.conj() * diag_2 * AD1q.conj()))
        
    #     H[1,1] =  np.real(np.sum(D2(D2(q)).conj() * grad,axis = None))
    #     H[1,1] += 2 * np.real( np.sum( AD2q.conj() * diag_1 * AD2q))
    #     H[1,1] += 2 * np.real( np.sum( AD2q.conj() * diag_2 * AD2q.conj()))
        
    #     # H = np.ndarray([[H_11, H_12],[H_21,H_22]])
    #     Hinv = np.linalg.pinv(H)
        
    #     # gr_1 = np.swapaxes(np.swapaxes(grad, 0, 1) * -2j*np.pi*np.arange(z.shape[0]), 0,1) / z.shape[0]
    #     # gr_2 = grad * -2j*np.pi*np.arange(z.shape[1]) / z.shape[1]
    #     gr_loc1 = np.real(np.sum(D1(q).conj() * grad,axis = None))
    #     gr_loc2 = np.real(np.sum(D2(q).conj() * grad,axis = None))
    #     gr = np.array([gr_loc1,gr_loc2])
    #     # noise = np.random.normal(0.0,1.0,size = 2)
    #     # gr = gr + 1.0 * noise / np.linalg.norm(noise) * np.linalg.norm(gr) * 0.1
    #     return(Hinv @ gr)
    
    def run_shifts(self,z):
        # we split the learning rate to stabilize numerically
        # full learining rate
        # fft_z_c = np.fft.fft2(z).conj()
        # A_norm_sq = np.prod(z.shape) * np.max(np.abs(fft_z_c))**2 * np.max(np.abs(self.par.window))**2
        
        print('Location correction:')
        norm_z = np.linalg.norm(z,'fro')
        z = z/norm_z
        
        fft_z_c = np.fft.fft2(z).conj()
        A_norm_sq = np.max(np.abs(fft_z_c))**2* np.max(np.abs(self.par.window))**2
        A_norm_sq *= np.prod(self.par.fourier_dimension)/ np.prod(z.shape)
        
        for r in range(self.par.R):
            print('Location ',r,' out of ', self.par.R)
            
            loc = self.par.locations[r]
            sqb_loc = self.sqb[:,:,r]
            sqb_loc = np.sqrt((sqb_loc**2 - self.epsilon) / norm_z**2 + self.epsilon)
            
            if self.learning_rate_type == 'optimal':
            # lr = np.prod(z.shape) *32 * np.pi**2 * A_norm_sq 
                lr = np.prod(z.shape) *32 * np.pi**2 * A_norm_sq 
                lr += 16 * np.pi**2 * np.sqrt(np.prod(z.shape) * A_norm_sq) * np.linalg.norm(sqb_loc,'fro')
                lr = 1.0/lr
            else:
                lr = self.learning_rate
            # lr = 10**-3
            
            grad_f = lambda x,forw,sqrt_meas: self.grad_shift(x,forw,sqrt_meas,sqb_loc,z,fft_z_c)
            
            obj_f = lambda x : self.obj_shift(x,z,sqb_loc)
            ...
            f1 = np.floor(loc_new[0]) // z.shape[0]
            loc_new[0] -= f1 * z.shape[0]
            
            f2 = np.floor(loc_new[1]) // z.shape[1]
            loc_new[1] -= f2 * z.shape[1]
            
            print('Old loc: ', loc, ' New loc: ', loc_new)
...
class blind_amplitude_flow:
    def __init__(self, 
        ...
        if 'AGP_shift' in kwargs.keys():
            self.AGP_shift = copy.deepcopy(kwargs['AGP_shift'])
        else:
            self.AGP_shift = types.SimpleNamespace(enable_AG=True,
                                control=0.5,
                                tau = 0.1,
                                AG_iterations = 20)
        ...
        if 'shift_subiterations' in kwargs.keys():
            self.shift_subiterations = kwargs['shift_subiterations']
        else:
            self.shift_subiterations = 100
        ...
        if 'shift_learning_rate' in kwargs.keys():
            self.shift_learning_rate = kwargs['shift_learning_rate']
        else:
            self.shift_learning_rate = 1.0
        
        if 'shift_learning_rate_type' in kwargs.keys():
            self.shift_learning_rate_type = kwargs['shift_learning_rate_type']
        else:
            self.shift_learning_rate_type = 'optimal'
        ...
        if 'grad_thr_shift' in kwargs.keys():
            self.grad_thr_shift = kwargs['grad_thr_shift']
        else:
            self.grad_thr_shift = 0    
        ...
        if 'update_shifts' in kwargs.keys():
            self.update_shifts = kwargs['update_shifts']
        else:
            self.update_shifts = False
        
        if 'update_shifts_period' in kwargs.keys():
            self.update_shifts_period = kwargs['update_shifts_period']
        else:
            self.update_shifts_period = 10
        
        if 'skip_shift_it' in kwargs.keys():
            self.skip_shift_it = kwargs['skip_shift_it']
        else:
            self.skip_shift_it = 0
        ...
        if self.track:
            ...
            if self.reweight:
                obj_norm = np.sum(np.abs(self.obj_tr)**2,axis = None)
                win_norm = np.sum(np.abs(self.win_tr)**2,axis = None)
    ...
    def run(self,
        ...
        if len(self.track_it) > 0:
            ...
            self.tracked_windows = np.zeros((0,w.shape[0],w.shape[1]),dtype = complex)
        ...
        for lp in range(self.number_of_iterations):
            ...
            if self.update_shifts and lp >= self.skip_shift_it and lp % self.update_shifts_period == 0:
                af_s = amplitude_flow(sqb = self.sqb,
                                      ptycho = af_o.par.copy(),
                                      AG_params = self.AGP_shift,
                ...
                # af_o.AG_par = self.AGP_shift
                # af_o.number_of_iterations = self.shift_subiterations
                # af_o.grad_threshold = self.grad_thr_shift
                self.par.locations = af_s.run_shifts(z)
                # af_o.AG_par = self.AGP_object
                # af_o.number_of_iterations = self.object_subiterations
                # af_o.grad_threshold = self.grad_thr_object
            ...
            if self.track:
                self.measurement_error = np.append(self.measurement_error,af_o.measurement_error[offset:])
                self.objective = np.append(self.objective,af_o.objective[offset:])
                self.object_error = np.append(self.object_error,af_o.object_error[offset:])
            ...
            if lp >= self.skip_win_it:
                ...
                if self.track:
                    self.measurement_error = np.append(self.measurement_error,af_w.measurement_error[1:])
                    self.objective = np.append(self.objective,af_w.objective[1:])                
                    self.window_error = np.append(self.window_error,af_w.object_error[1:])
            else:
                if self.track:
                    self.measurement_error = np.append(self.measurement_error,np.repeat(self.measurement_error[-1], self.window_subiterations))
                    self.objective = np.append(self.objective,np.repeat(self.objective[-1], self.window_subiterations))                
                    self.window_error = np.append(self.window_error,np.repeat(0, self.window_subiterations)) 
            ...
            if count < len(self.track_it):
                if lp == self.track_it[count]:
                    self.tracked_objects = np.append(self.tracked_objects,[util.align_objects(self.obj_tr,z,self.par.mask)],axis= 0)
                    self.tracked_windows = np.append(self.tracked_windows,[util.align_objects(self.win_tr,w,np.ones((w.shape[0],w.shape[1])))],axis= 0)
                    count += 1
