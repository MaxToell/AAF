...
def loc_Fermat_spiral(obj_s, win_s, seed_size, float_shift):
    ...
    for n in range(N):
        ...
        phi = n * phi_0
        x = r * math.cos(phi) + 0.5*s1  
        y = r * math.sin(phi) + 0.5*s2
...
def loc_spiral(obj_s, win_s, seed_size, float_shift):
    ...
    N = np.ceil( ( ( 0.5*np.max([s1 + win_s[0],s2 + win_s[1]]))/ seed_size)**2 ).astype(np.int32)  
    phi_0 = 8 * math.pi / (1 + math.sqrt(5))**2 
    for n in range(N):
        r = seed_size * math.sqrt(n)
        phi = np.sqrt(n) * phi_0
    ...
    def __init__(self, **kwargs):
        ...
        if 'locations' in kwargs.keys():
            ...
            if kwargs['loc_type'] == 'grid':
                ...
                self.locations = loc_spiral(self.object_shape, self.window_shape, kwargs['fermat_seed_size'],self.float_shift)
                # self.locations = loc_Fermat_spiral(self.object_shape, self.window_shape, kwargs['fermat_seed_size'],self.float_shift)
            else:
