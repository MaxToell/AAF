# -*- coding: utf-8 -*-
"""
Created on Wed Nov 23 17:26:39 2022
...
def generate_noise(b,window,fourier_dimension, nop = 10**12):
    scaling = nop / np.linalg.norm(window,'fro')**2 / np.prod(fourier_dimension)
    b_scaled =  scaling * b;
    b_n = np.random.poisson(b_scaled, b_scaled.shape) / scaling
    return b_n

# b_n = b
b_n = generate_noise(b, window, par.fourier_dimension, 10**-1)

print('SNR: ', 10*np.log10(np.sum(np.abs(b)**2)/np.sum(np.abs(b - b_n)**2) ))
...
AG_par = types.SimpleNamespace(enable_AG=False,
                    control=0.5,
                    tau = 0.3,
                    AG_iterations = 2)
