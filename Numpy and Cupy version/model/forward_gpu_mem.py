# -*- coding: utf-8 -*-
"""
Created on Sun Dec 18 14:45:06 2022
...
def loc_grid_noncirc(obj_s, win_s, shift_s):
    locations_1 = cp.array(range(0,obj_s[0]-win_s[0]+1, shift_s[0]))
    locations_2 = cp.array(range(0,obj_s[1]-win_s[1]+1, shift_s[1]))
    locations = cp.zeros((len(locations_1)*len(locations_2),2),dtype=int)
    locations[:,0] = cp.repeat(locations_1,len(locations_2))
    locations[:,1] = cp.tile(locations_2,len(locations_1))
    
    return locations    

def loc_grid_circ(obj_s, shift_s):
    locations_1 = cp.array(range(0,obj_s[0], shift_s[0]))
    locations_2 = cp.array(range(0,obj_s[1], shift_s[1]))
    locations = cp.zeros((len(locations_1)*len(locations_2),2),dtype=int)
    locations[:,0] = cp.repeat(locations_1,len(locations_2))
    locations[:,1] = cp.tile(locations_2,len(locations_1))
...
def loc_Fermat_spiral(obj_s, win_s, seed_size):
    ...
    N = cp.ceil(2*(0.5*cp.max([s1,s2])/ seed_size)**2).astype(cp.int32)  
    phi_0 = 8 * math.pi / (1 + math.sqrt(5))**2 
    for n in range(N):
        r = seed_size * math.sqrt(n)
        phi = n * phi_0
        x = cp.round(r * math.cos(phi) + 0.5*s1).astype(cp.int32)  
        y = cp.round(r * math.sin(phi) + 0.5*s2).astype(cp.int32)
...
class ptycho:
    def __init__(self, **kwargs):
        ...
        self.obj = cp.zeros(self.object_shape,dtype = complex)
        ...
        if 'window' in kwargs.keys():
            ...
            dummy_window = cp.zeros(kwargs['window_shape'],dtype = complex)
        ...
        if 'locations' in kwargs.keys():
            assert isinstance(kwargs['locations'], cp.ndarray), "Locations is not of type numpy.ndarray"
            ...
            if kwargs['loc_type'] == 'grid':
                ...
                assert isinstance(kwargs['shift'], tuple) or isinstance(kwargs['shift'], int), "Shift size is not tuple or int"
                ...
                if self.circular:
                    self.locations = loc_grid_circ(self.object_shape, shift)
                else:
                    self.locations = loc_grid_noncirc(self.object_shape, self.window_shape, shift)
        ...
        self.mask = cp.zeros(self.object_shape)
        ...
        #     assert isinstance(kwargs['detector_mask'], cp.ndarray) or isinstance(kwargs['detector_mask'], int) or isinstance(kwargs['detector_mask'], tuple) , "detector_mask is not of type numpy.ndarray, tuple or int"
            
        #     if isinstance(kwargs['detector_mask'], cp.ndarray):
        ...
        #         assert cp.sum(cp.isin(kwargs['detector_mask'],[0,1])) == cp.prod(kwargs['detector_mask'].shape), "Detector mask should be either 0 or 1"
        ...
        #         self.detector_mask =  cp.zeros(self.fourier_dimension,dtype=complex)
        ...
        # else:
        #     self.detector_mask =  cp.ones(self.fourier_dimension,dtype=complex)
    ...
    def check_outbound(self):
        ...
        if not self.circular:
            min_loc = cp.min(self.locations, axis = 0)
            max_loc = cp.max(self.locations, axis = 0)
            ...
            assert outbound, "Location out of bound"

    def set_object(self, obj):
        assert isinstance(obj, cp.ndarray), "Window is not of type numpy.ndarray"
    ...
    def set_window(self, window):
        assert isinstance(window, cp.ndarray), "Window is not of type numpy.ndarray"
    ...
    def forward_2D_os(self, z):
        ...
        out_padded = cp.zeros(self.fourier_dimension,dtype=complex)
        out_padded[:self.window_shape[0],:self.window_shape[1]] = self.window * z
        
        # place middle of the illumineted area to the 0
        out_padded = cp.roll(out_padded, -r1, axis=0)
        out_padded = cp.roll(out_padded, -r2, axis=1)
        
        out_fft = cp.fft.fft2(out_padded)
        
        out_fft = cp.roll(out_fft, h1, axis=0)
        out_fft = cp.roll(out_fft, h2, axis=1)
    ...
    def forward_adj_2D_os(self,forw):
        ...
        unzoomed_padded = cp.roll(unzoomed_padded, -h2, axis=1)
        unzoomed_padded = cp.roll(unzoomed_padded, -h1, axis=0)
        out_fft = cp.fft.ifft2(unzoomed_padded)*cp.prod(self.fourier_dimension)    
        
        r1 = d1//2
        r2 = d2//2
        
        out_fft = cp.roll(out_fft, r2, axis=1)
        out_fft = cp.roll(out_fft, r1, axis=0)
    ...
    # def meas_err_sq(self,z,sqb, epsilon):
    #     delta1 = self.window_shape[0]
    #     delta2 = self.window_shape[1]
        
    #     # forw = cp.zeros((self.fourier_dimension[0],self.fourier_dimension[1],self.R),dtype = complex)
    ...
    #         forw = self.forward_2D_os(z_r)
    #         meas = cp.sqrt( cp.abs(forw)**2 + epsilon)
    #         diff += cp.abs(meas - sqb[:,:,r])**2
    ...
    def forward_adj_2D_pty(self,forw):
        ...
        z = cp.zeros((self.object_shape[0],self.object_shape[1]),dtype = complex)
        
        for r in range(self.R):
            loc = self.locations[r,:] 
            
            if self.circular:
                z_r = cp.zeros(self.object_shape,dtype = complex)
                z_r[:delta1,:delta2] = self.forward_adj_2D_os(forw[:,:,r])       
                z_r = cp.roll(z_r,loc[0],0)
                z_r = cp.roll(z_r,loc[1],1)
                z += z_r
            else:
