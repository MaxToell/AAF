...
from scipy.ndimage.interpolation import shift as sht
def loc_grid_noncirc(obj_s, win_s, shift_s):
    print(str(obj_s),str(win_s),str(shift_s))
    locations_1 = np.array(range(0,obj_s[0]-win_s[0]+1, int(shift_s[0])))
    locations_2 = np.array(range(0,obj_s[1]-win_s[1]+1, int(shift_s[0])))
    ...
    def __init__(self, **kwargs):
        ...
        if 'locations' in kwargs.keys():
            ...
            if kwargs['loc_type'] == 'grid':
                ...
                assert isinstance(kwargs['shift'], tuple) or isinstance(kwargs['shift'], float), "Shift size is not tuple or int"
                
                if isinstance(kwargs['shift'], tuple):
                    assert len(kwargs['shift']) == 2, "Shift tuple is not 2D"
                    shift = kwargs['shift']
                
                if isinstance(kwargs['shift'], float):
